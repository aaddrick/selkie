...
- [Specific change: e.g., "adding a Zig-specific anti-pattern for memory leaks"]
...
| Type | Target | Example |
|------|--------|---------|
| **Anti-pattern** | Agent definition | "NEVER use `page_allocator` in tests — use `testing.allocator`" |
| **Missing guidance** | Skill content | Add edge case handling for cmark-gfm C FFI |
| **New trigger** | Skill description | Add symptom that should invoke this skill |
| **Hook gap** | settings.json / hook script | Build check not catching a file type |
